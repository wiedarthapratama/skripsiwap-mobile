...
class WCityRegion extends StatefulWidget {
  const WCityRegion({Key? key, required this.id}) : super(key: key);

  final int id;

  @override
  State<WCityRegion> createState() => _WCityRegionState();
}

class _WCityRegionState extends State<WCityRegion> {
  bool isLoading = false;
  List<CityModel> data = [], result = [];
  ...
  void _init() async {
    ...
    final response = await repository.getCity(id: widget.id);
    final List<CityModel> data = response.fold((l) => [], (r) => r);
    isLoading = false;
    this.data = data;
    result = data;
    setState(() {});
