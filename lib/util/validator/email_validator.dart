...
enum EmailType { none, alphabetic, numeric, alphaNumeric }

class EmailValidator {
  static int _index = 0;
  static const String _atomCharacters = "!#\$%&'*+-/=?^_`{|}~";
  static EmailType _domainType = EmailType.none;
  ...
  static bool _isDomain(String c, bool allowInternational) {
    if (c.codeUnitAt(0) < 128) {
      if (_isLetter(c) || c == '-') {
        _domainType = EmailType.alphabetic;
        return true;
      }

      if (_isDigit(c)) {
        _domainType = EmailType.numeric;
        return true;
      }
  ...
  static bool _skipDomain(
    ...
    if (_domainType == EmailType.numeric) {
      return false;
    }
